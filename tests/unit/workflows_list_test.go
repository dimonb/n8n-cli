...
func TestListWorkflowsCommand(t *testing.T) {
	...
	setupTestCommand := func() *cobra.Command {
		...
		cmd.Flags().IntP("limit", "l", 0, "Maximum number of workflows to return (default: single page)")
	...
	t.Run("MaxWorkflowsLimit constant is exported and equals 100", func(t *testing.T) {
		assert.Equal(t, 100, n8n.MaxWorkflowsLimit, "MaxWorkflowsLimit should be 100")
	})
}
